<template>
    <div class="mx-auto">
        ...
        <section class="mb-12">
            ...
            <div class="mb-6">
                <Input v-model="globalFilter" placeholder="Пошук..." class="max-w-sm"/>
            </div>
            ...
            <div class="rounded-lg border shadow-sm overflow-x-auto">
                <Table>
                    <TableHeader>
                        <TableRow>
                            <TableHead  v-for="header in table.getFlatHeaders()"
                            :key="header.id"
                            :class="[
                            ...
                            <SelectContent>
                                <SelectGroup>
                                    ...
                                    <SelectItem
                                        ...
                                        :key="car.vin"
                                        :value="car?.carModel?.modelId?.toString()" v-if="selectedCarClient?.clientCars?.length">
                            ...
                            <SelectContent>
                                <SelectGroup>
                                    ...
                                    <SelectItem
                                        ...
                                        :value="station.stationId.toString()" v-if="selectedServicesByBrand?.length">
                                        {{ station.stationName }}
                                    </SelectItem>
                                    <div
                                        class="text-gray-400 text-center"
                        ...
                        <Select
                            ...
                            <SelectContent>
                                <SelectGroup>
                                    ...
                                    <SelectItem value="Ожидание">Очікування</SelectItem>
                                    <SelectItem value="В работе">В роботі</SelectItem>
                                    <SelectItem value="Завершен">Завершено</SelectItem>
                                    <SelectItem value="Отменен">Скасовано</SelectItem>
                                </SelectGroup>
                            </SelectContent>
                        </Select>
                    </div>
    ...
    if (selectedBrand) {
        ...
        selectedServicesByBrand.value = serviceStations.value.filter((station) => {
            return station.specializations.some((service) => {
                return selectedCarClient.value?.clientCars?.some((car) => car.carBrand && car.carBrand.brandId === service.brandId) || false;
            });
        });
...
const openDeleteModal = (type: "application", id: number) => {
    deleteId.value = id;
